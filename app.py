...
def upload_file():
    if request.method == 'POST':
        ...
        if file:
            ...
            # Esperar processamento (podes melhorar com websocket ou polling)
            time.sleep(15)  # Espera simplificada para o job completar

            # Buscar info do job atualizado
            job_result = requests.get(
                f'https://api.cloudconvert.com/v2/jobs/{job_id}',
                headers={'Authorization': f'Bearer {API_KEY}'}
            ).json()

            # Buscar URLs dos arquivos gerados
            export_tasks = [t for t in job_result['data']['tasks'] if t['operation'] == 'export/url']
            download_links = []
    ...
    return '''
    <!doctype html>
    <title>Conversor de PDF para PNG e JPG</title>
    <h1>Upload do PDF</h1>
